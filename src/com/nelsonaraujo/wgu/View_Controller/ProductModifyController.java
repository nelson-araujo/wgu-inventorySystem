 ...
 * Controls for ProductModifyView.fxml
...
public class ProductModifyController {
    ...
    /** productModifyController constructor.
     * Passes inventory and the product selected.
     * @param primaryInv Main inventory.
     * @param productSelected Product selected to be modified.
     */
    public ProductModifyController(Inventory primaryInv, Product productSelected) {
     ...
     * Initialize the all parts and product parts table and all fields.
     */
    public void initialize(){
        generateInitialProductPartsTable();
        generateInitialAllPartsTable();

        // Set all all fields
        productId.setText(Integer.toString(productSelected.getProductId()));
        productName.setText(productSelected.getProductName());
        productInventoryLevel.setText(Integer.toString(productSelected.getProductStock()));
        productPrice.setText(Double.toString(productSelected.getProductPrice()));
        productInventoryMaximum.setText(Integer.toString(productSelected.getProductMax()));
        productInventoryMinimum.setText(Integer.toString(productSelected.getProductMin()));

    }

    /** Populate the initial all parts table.
     * Retrieve all parts fromm inventory, remove the parts already assigned, and populate the initial all parts table. 
     */
    @FXML
    private void generateInitialAllPartsTable(){
        allPartsList.setAll(primaryInv.getAllParts()); // Assign all parts to list

        // Loop through productPartsList to remove them from allPartsList
        for(int i=0 ; i < productPartsList.size() ; i++){

            // Loop through allPartsList to find part
            for( int j=0 ; j < allPartsList.size() ; j++){

                // Find the parts in allPartsList
                if(productPartsList.get(i).getPartId() == allPartsList.get(j).getPartId()){

                    // Remove part from allParts
                    allPartsList.remove(j);
                }
            }
        }

        allPartsTable.setItems(allPartsList); // Add all the parts to the table
        allPartsTable.refresh(); // Refresh the table

    }

    /** Populate the initial product parts table.
     * Retrieve all the product parts and populate the table.
     */
    @FXML
    private void generateInitialProductPartsTable(){
        productPartsList.setAll(productSelected.getAllAssociatedParts());
    ...
    /** Update product.
     * Update product based on the user input.
     */
    @FXML
    private void updateProduct(){
        // Create temp product with user selections
        ...
        for(int i=0 ; i< productPartsList.size() ; i++){
            updatedProduct.addAssociatedPart(productPartsList.get(i));
        }

        // Update inventory
        primaryInv.updateProduct(updatedProduct.getProductId(),updatedProduct);
